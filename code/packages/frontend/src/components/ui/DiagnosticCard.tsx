...
export function DiagnosticCard({
  ...
  return (
    <div className="rounded-lg border border-[var(--lfb-border)] bg-white shadow-[var(--lfb-shadow-sm)]">
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {summary && <div className="mt-0.5 text-sm leading-relaxed text-black/60">{summary}</div>}
          {hasBody && (
            <button
              type="button"
              aria-expanded={open}
              onClick={() => setOpen((o) => !o)}
              className="-ml-1 mt-1.5 flex items-center gap-1 rounded px-1 py-0.5 text-xs text-black/50 transition-colors duration-150 hover:bg-[var(--lfb-hover)] hover:text-black"
            >
      ...
      {hasBody && open && (
        <div className="border-t border-[var(--lfb-border)] bg-[var(--lfb-surface-2)] px-4 py-3 pl-12 text-sm leading-relaxed text-black/70">
          {children}
        </div>
      )}
    </div>
